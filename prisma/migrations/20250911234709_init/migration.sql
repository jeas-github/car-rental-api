...
CREATE TABLE `rental_points` (
    ...
    UNIQUE INDEX `rental_points_name_key`(`name`),
...
CREATE TABLE `cars` (
    ...
    `category` ENUM('econômica', 'sedan', 'suv', 'luxo', 'minivan') NOT NULL DEFAULT 'econômica',
    `status` ENUM('disponível', 'alugado', 'manutenção', 'desativado') NOT NULL DEFAULT 'disponível',
...
CREATE TABLE `rentals` (
    ...
    `status` ENUM('pendente', 'ativo', 'concluído', 'cancelado') NOT NULL DEFAULT 'pendente',
    `created_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `updated_at` DATETIME(3) NOT NULL,
